...
while true; do
  ...
  while true; do
	...
	curl -s https://api.twitch.tv/kraken/streams/$1?client_id=(twitchclientid)|grep -q live&& break
  ...
  ffmpeg -reconnect_at_eof 1 -reconnect_streamed 1 -reconnect_delay_max 8  -i "$M3U8_URL" -codec copy   -f hls -hls_time 3600 -hls_list_size 0 "$5$FNAME" > "$5$FNAME.log" 2>&1
